...
VM_IMAGE_CHECKSUMS_URL=$VM_IMAGE_URL_BASE/SHA512SUMS
...
if [ -f "$VM_IMAGE_FN" ]; then
    ...
    TDIR=$(mktemp -d)
    echo "Downloading SHA512 checksums from $VM_IMAGE_CHECKSUMS_URL"
    curl --fail -L $VM_IMAGE_CHECKSUMS_URL | grep "$VM_IMAGE_FN" > $TDIR/SHA512SUMS
    echo "Downloading $VM_IMAGE_FN from $VM_IMAGE_URL"
    curl --fail -Lo $TDIR/$VM_IMAGE_FN $VM_IMAGE_URL
    echo "Checking that $VM_IMAGE_FN matches checksum"
    (cd $TDIR && sha512sum -c SHA512SUMS)
